...
@Component({
  selector: 'app-logapp-poweruser',
  templateUrl: './logapp-poweruser.component.html',
  styleUrls: ['./logapp-poweruser.component.css']
})
export class LogappPoweruserComponent {

  nombre = localStorage.getItem('nombre');
  responseLAPPDto: ResponseLAPPDto[] = [];
  ...
  constructor(private luservice: LuService) {
    this.dataSource = new MatTableDataSource(this.responseLAPPDto);
  }

  ngAfterViewInit() {

    this.luservice.listarLogsAPP().subscribe({
      next: (data: ResponseLAPPDto[]) => {
        console.log(data);
        this.responseLAPPDto = data;
        this.dataSource = new MatTableDataSource(this.responseLAPPDto);
        // @ts-ignore
